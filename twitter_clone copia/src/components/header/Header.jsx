import "./index.css"


const Header = () => {
    return (
        <div className="Header">
            <img src="data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAB4AAAAeCAYAAAA7MK6iAAAACXBIWXMAAAsTAAALEwEAmpwYAAAAPklEQVR4nO3VwQkAIAxD0Qxlpu02dqFuoBN4ECpe/oOcC4FQCUA3R6Wj1uPkl8MjarY3BhwxJ6CN+U4AdGcDEPypntdQSp4AAAAASUVORK5CYII=" alt = "burger_menu"/>
            <img src="data:image/png;base64," alt ="twitter_icon" />
            <img src="data:image/png;base64," alt ="sparkling" />
        </div>

    );
}; //#359bf0

export default Header